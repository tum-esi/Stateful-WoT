...
export function launchCoffeeMachineServer(WoT: typeof globalThis.WoT, servNum: number, deviceNum: number) {
  ...
  const td: any = {
    ...
    "@type": "Thing",
    title: `Virtual Coffee Machine ${servNum}_${deviceNum}`,
  ...
  (async () => {
    ...
    thing.setPropertyReadHandler("taskStatus", () => {
      const services = service.getSnapshot().context["_spawnedServices"]
      const servicesKeys = Object.keys(services);
      const currentTask = servicesKeys.find((serviceId) => serviceId === `busy.${service.getSnapshot().context._spawnedServicesCounter - 1}`)
      if (currentTask) return services[currentTask].getSnapshot().value
    ...
    thing.setActionHandler("makeDrink", async (inputData, options) => {
      ...
      if (!["wrapper.coffeeOutlet.idle"].some(currentState.matches)) {
        ...
        const payload = await inputData.value()
        ...
        if(payload !== undefined && payload.drinkId !== null && !possibleDrinks.includes(payload.drinkId)) throw new Error(payload.drinkId + " is not a possible drink")
        service.send({
          type: "taskReceived",
    ...
    thing.setEventUnsubscribeHandler("outOfResource", async () => {
    ...
    service.onTransition((state, context) => {
      ...
      if (service.getSnapshot().value["wrapper"]["coffeeOutlet"] === "busy" && lastState.value["wrapper"]["coffeeOutlet"] === "idle") {
        let currentCounter = service.getSnapshot().context._spawnedServicesCounter
        const currentTask = service.getSnapshot().context["_spawnedServices"][`busy.${currentCounter - 1}`]
        if (currentTask) currentTask.onTransition((state) => {
          if(state.changed) {
            console.log(`${td.title}:`)
            console.log(`Recieved Task Event: ${JSON.stringify(state.event)}`);
            console.log(`Current Task State: ${JSON.stringify(state.value)}`);
          }
    ...
    const thingTD = thing.getThingDescription()
  try {
    service.start();
    console.log("\n------------------------------------------------------------------\n")
    const promise = thing.expose();
    await promise
    const thingTD = thing.getThingDescription()
    const baseHref = thingTD.properties?.allAvailableResources?.forms[0].href.replace("/properties/allAvailableResources", "")
    console.log(`${thingTD.title} is exposed on ${baseHref}`)
    } catch (e) {
      console.log(`Error in ${thingTD.title}: ${e}`)
      console.log(`TD: ${JSON.stringify(thingTD)}`)
    }
  })()
}
