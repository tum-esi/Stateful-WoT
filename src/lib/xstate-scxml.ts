...
export class SCXML2XStateCodeParser {
   ...
   */
  protected readonly XStateMachineOptions: InternalMachineOptions<
    any,
    any,
    any
  > = {
  ...
  public async generateXStateCode(
    ...
    this.finalXStateCode = `import { createMachine, interpret, actions, spawn, AnyStateMachine, sendParent } from 'xstate';
...
function delayToMs (delay: string | number): number {
  ...
  if (secondsMatch !== null) {
    ...
    const secondsPart = secondsMatch[3] !== undefined && secondsMatch[3] !== ''
      ? parseInt(secondsMatch[3], 10) * 1000
      : 0
...
`;
    // Prettifying code
    this.finalXStateCode = prettier.format(this.finalXStateCode, {
      parser: "typescript",
    });
   ...
   */
  public async generateXStateMachineFactory(
    scxml: string | Node,
  ): Promise<string> {
    // parse SCXML Node
    ...
    this.intermediaryXStateMachineFactoryCode = `createMachine(
...
`;
    ...
    this.finalXStateMachineFactoryCode = template(
    ...
    do {
      this.finalXStateMachineFactoryCode = template(
        this.finalXStateMachineFactoryCode,
        this.variableMap,
        { before: '"{{', after: '}}"' },
      );
  ...
  protected async parseSCXML(scxml: string | Node): Promise<void> {
    ...
    // Get SCXML name
    const nameAttr = select("./@name", this.scxmlNode, true) as
      | Attr
      | undefined;
    if (nameAttr?.value !== undefined)
      this.XStateMachineConfig.id = nameAttr.value;

    // Get Initial if available
    const initialAttr = select("./@initial", this.scxmlNode, true) as
      | Attr
      | undefined;
    this.XStateMachineConfig.initial =
      initialAttr?.value !== undefined ? initialAttr.value : undefined;
    ...
    if (this.XStateMachineConfig.initial === undefined) {
      for (let i = 0; i < this.scxmlNode.childNodes.length; i++) {
        ...
        if (
          ...
          this.XStateMachineConfig.initial = tempIdAttr.value;
          break;
        }
      }
      if (this.XStateMachineConfig.initial === undefined)
        throw Error(
          "<scxml> node does not contain a <state>, <parallel> or <final> element with an ID that can be considered as initial state!",
        );
    }
    ...
    for (let i = 0; i < this.scxmlNode.childNodes.length; i++) {
      ...
      if (childNode.nodeType === childNode.ELEMENT_NODE) {
        switch (childNode.nodeName) {
          case "state":
            if (this.XStateMachineConfig.states === undefined)
              this.XStateMachineConfig.states = {};
            await this.addNormalState(
              childNode,
              this.XStateMachineConfig.states,
            );
            break;
  ...
  protected async addNormalState(
    ...
    // Check if it has an ID
    const tempIdAttr = select("./@id", childStateNode, true) as
      | Attr
      | undefined;
    let id: string;
    if (tempIdAttr?.value !== undefined) id = tempIdAttr.value;
    else {
      id = `${NORMAL_STATE_GENERIC_ID_PREFIX}${this.NORMAL_STATE_GENERIC_COUNTER}`;
      this.NORMAL_STATE_GENERIC_COUNTER++;
    }
  ...
  protected async addParallelState(
    ...
    // Check if it has an ID
    const tempIdAttr = select("./@id", childParallelNode, true) as
      | Attr
      | undefined;
    let id;
    if (tempIdAttr?.value !== undefined) id = tempIdAttr.value;
    else {
      id = `${PARALLEL_STATE_GENERIC_ID_PREFIX}${this.PARALLEL_STATE_GENERIC_COUNTER}`;
      this.PARALLEL_STATE_GENERIC_COUNTER++;
    }
  ...
  protected async addTransition(
    ...
    const eventAttr = select("./@event", transitionNode, true) as
      ...
      | undefined;
    const eventName =
      eventAttr?.value !== undefined ? eventAttr.value : undefined;
    ...
    const condAttr = select("./@cond", transitionNode, true) as
      ...
      | undefined;
    let cond;
    if (condAttr?.value !== undefined && condAttr.value === '"clockCond"')
      cond = { type: JSON.parse(condAttr.value) };
    else
      cond =
        condAttr?.value !== undefined
          ? this.conditionParser(condAttr.value)
          : undefined;

    // Get target if available
    const targetAttr = select("./@target", transitionNode, true) as
      | Attr
      | undefined;
    const target =
      targetAttr?.value !== undefined ? targetAttr.value : undefined;

    // Get type if available
    const typeAttr = select("./@type", transitionNode, true) as
      | Attr
      | undefined;
    ...
    for (let i = 0; i < transitionNode.childNodes.length; i++) {
      ...
      if (childNode.nodeType === childNode.ELEMENT_NODE) {
        ...
        await this.addExecutableContent(
          transitionNode,
          transitionObject.actions,
        );
      }
  ...
  protected getInitialState(stateNode: Node, NodeObj: any): void {
    // Get Initial if available
    const initialAttr = select("./@initial", stateNode, true) as
      | Attr
      | undefined;
    NodeObj.initial =
      initialAttr?.value !== undefined ? initialAttr.value : undefined;
    ...
    if (NodeObj.initial === undefined) {
      for (let i = 0; i < stateNode.childNodes.length; i++) {
        ...
        if (
          ...
          if (tempIdAttr == null) continue;
          // If id is found, add it to xstate
          NodeObj.initial = tempIdAttr.value;
          break;
        }
      }
    }
  ...
  protected async addFinalState(
    ...
    if (idAttr?.value !== undefined) id = idAttr.value;
      ...
      id = `${FINAL_STATE_GENERIC_ID_PREFIX}${this.FINAL_STATE_GENERIC_COUNTER}`;
      this.FINAL_STATE_GENERIC_COUNTER++;
    }
   ...
   * @param parentNodeObj
   */
  protected async addOnEntry(
    childOnEntryNode: Node,
    parentNodeObj: any,
  ): Promise<void> {
   ...
   * @param parentNodeObj
   */
  protected async addOnExit(
    childOnExitNode: Node,
    parentNodeObj: any,
  ): Promise<void> {
  ...
  protected addHistoryPseudoState(
    historyStateNode: Node,
    parentNodeObj: any,
  ): void {
    const historyObj: {
      type: "history";
    ...
    // Check if it has an ID
    const tempIdAttr = select("./@id", historyStateNode, true) as
      | Attr
      | undefined;
    let id;
    if (tempIdAttr?.value !== undefined) id = tempIdAttr.value;
    else {
      id = `${HIST_STATE_GENERIC_ID_PREFIX}${this.HIST_STATE_GENERIC_COUNTER}`;
      this.HIST_STATE_GENERIC_COUNTER++;
    }
    // Check if type is available
    const typeAttr = select("./@type", historyStateNode, true) as
      | Attr
      | undefined;
    const type = (
      typeAttr?.value !== undefined ? typeAttr.value : undefined
    ) as "shallow" | "deep" | undefined;

    let target;
    if (type !== undefined) historyObj.history = type;
    for (let i = 0; i < historyStateNode.childNodes.length; i++) {
      const childNode = historyStateNode.childNodes[i];
      if (
        childNode.nodeType === childNode.ELEMENT_NODE &&
        childNode.nodeName === "transition"
      ) {
        const targetAttr = select("./@target", childNode, true) as
          | Attr
          | undefined;
        target = targetAttr?.value !== undefined ? targetAttr.value : undefined;
        break;
      }
    }
   ...
   */
  protected async addStateChildren(
    node: Node,
    nodeId: string,
    currentNodeObj: any,
  ): Promise<void> {
  ...
  protected async addFinalStateChildren(
    finalNode: Node,
    currentNodeObj: any,
  ): Promise<void> {
    // Process Element Child Nodes
   ...
   * @param actionsArray
   */
  protected async addExecutableContent(
    selfNode: Node,
    actionsArray: any[],
  ): Promise<void> {
   ...
   */
  protected addAssign(
    childAssignNode: Node,
    actionsArray: any,
    cond?: string | "else",
  ): void {
    // Get location
    const locationAttr = select("./@location", childAssignNode, true) as
      | Attr
      | undefined;
    if (locationAttr?.value === undefined)
      throw new Error("<assign> node does not have a location attribute.");

    const location = locationAttr.value;

    // Get value or expression to be assigned
    const exprAttr = select("./@expr", childAssignNode, true) as
      | Attr
      | undefined;
    let expr;
    ...
    if (expr === undefined)
      throw new Error(
        "Could not find an expression for value assignment in <assign> node",
      );
    ...
    if (
      parsedLoction.body[0].type === "ExpressionStatement" &&
      parsedLoction.body[0].expression.type === "MemberExpression" &&
      parsedLoction.body[0].expression.object.type === "Identifier"
    ) {
      ...
      this.variableMap[
        `${ASSIGN_PREFIX}${this.ASSIGN_COUNTER}`
      ...
      if (
        parsedTree.body?.length === 1 &&
        parsedTree.body[0].expression?.type !== undefined &&
        parsedTree.body[0].expression.type === "Literal"
      ) {
        this.variableMap[
          `${ASSIGN_PREFIX}${this.ASSIGN_COUNTER}`
        ] = `assign({${location}: ${expressionModded}})`;
      } else {
        this.variableMap[
          `${ASSIGN_PREFIX}${this.ASSIGN_COUNTER}`
        ] = `assign({${location}: (context: any, event: any) => {return ${expressionModded}}})`;
      }
    }
  ...
  protected addIfCondition(ifNode: Node, actionsArray: any[]): void {
    ...
    if (condAttr?.value === undefined)
      throw new Error("<if> node does not have a cond attribute.");

    let cond = this.conditionParser(condAttr.value);

    const condsArray: any[] = [];
    let condActionObj: { cond?: string; in?: string; actions: any[] } = {
      actions: [],
    };
    ...
    for (let i = 0; i < ifNode.childNodes.length; i++) {
      ...
      switch (childNode.nodeName) {
        ...
        case "elseif":
          ...
          if (condAttr?.value === undefined)
            throw new Error("<elseif> node does not have a cond attribute.");
  ...
  protected addLog(logNode: Node, actionsArray: any[]): void {
    ...
    if (expr !== undefined) {
      ...
      if (label !== undefined)
        this.variableMap[
          `${LOG_PREFIX}${this.LOG_COUNTER}`
        ] = `log((context: any, event: any, {_event}) => {return ${expr}}, "${label}")`;
      else
        this.variableMap[
          `${LOG_PREFIX}${this.LOG_COUNTER}`
        ] = `log((context: any, event: any, {_event}) => {return ${expr}})`;
    } else {
      if (label !== undefined)
        this.variableMap[
          `${LOG_PREFIX}${this.LOG_COUNTER}`
  ...
  protected addRaise(raiseNode: Node, actionsArray: any[]): void {
    ...
    if (eventAttr?.value === undefined)
      throw Error("<raise> node does not contain event attribute");
  ...
  protected addSend(sendNode: Node, actionsArray: any[]): void {
    ...
    const idlocationAttr = select("./@idlocation", sendNode, true) as
      | Attr
      | undefined;
    ...
    if (idString !== undefined) sendOptions.id = idString;
      ...
      this.variableMap[
        `${SEND_IDEXPR_PREFIX}${this.SEND_IDEXPR_COUNTER}`
      ] = `(context: any, event: any, { _event }) => {return ${idExpr}}`;
    ...
    if (delayNumber !== undefined) sendOptions.delay = delayNumber;
      ...
      if (this.XStateMachineOptions?.delays === undefined)
        this.XStateMachineOptions.delays = {};
      this.variableMap[
        `${SEND_DELAYEXPR_PREFIX}${this.SEND_DELAYEXPR_COUNTER}`
...
`;
      sendOptions.delay =
        `{{${SEND_DELAYEXPR_PREFIX}${this.SEND_DELAYEXPR_COUNTER}}}` as any;
      this.SEND_DELAYEXPR_COUNTER++;
    }

    // add target if available
    if (targetString !== undefined && targetString !== "parent")
      sendOptions.to = targetString;
    else if (targetExpr !== undefined) {
      this.variableMap[
        `${SEND_TARGETEXPR_PREFIX}${this.SEND_TARGETEXPR_COUNTER}`
      ] = `(context: any, event: any, { _event }) => {return ${targetExpr}}`;
    ...
    if (targetString !== "parent") {
      if (eventString !== undefined && data === undefined) {
        this.variableMap[
          `${SEND_PREFIX}${this.SEND_COUNTER}`
        ] = `send('${eventString}'`;
      } else if (eventString !== undefined && data !== undefined) {
        this.variableMap[
          `${SEND_PREFIX}${this.SEND_COUNTER}`
        ] = `send((context: any, event: any, {_event}) => ({type: '${eventString}', data: ${data}})`;
      } else if (eventExpr !== undefined && data === undefined) {
        this.variableMap[
          `${SEND_PREFIX}${this.SEND_COUNTER}`
        ] = `send((context: any, event: any, {_event}) => ({type: ${eventExpr}})`;
      } else if (eventExpr !== undefined && data !== undefined) {
        this.variableMap[
          `${SEND_PREFIX}${this.SEND_COUNTER}`
        ] = `send((context: any, event: any, {_event}) => ({type: ${eventExpr}, data: ${data}})`;
      }
    } else {
      if (eventString !== undefined && data === undefined) {
        this.variableMap[
          `${SEND_PREFIX}${this.SEND_COUNTER}`
        ] = `sendParent('${eventString}'`;
      } else if (eventString !== undefined && data !== undefined) {
        this.variableMap[
          `${SEND_PREFIX}${this.SEND_COUNTER}`
        ] = `sendParent((context: any, event: any, {_event}) => ({type: '${eventString}', data: ${data}})`;
      } else if (eventExpr !== undefined && data === undefined) {
        this.variableMap[
          `${SEND_PREFIX}${this.SEND_COUNTER}`
        ] = `sendParent((context: any, event: any, {_event}) => ({type: ${eventExpr}})`;
      } else if (eventExpr !== undefined && data !== undefined) {
        this.variableMap[
          `${SEND_PREFIX}${this.SEND_COUNTER}`
        ] = `sendParent((context: any, event: any, {_event}) => ({type: ${eventExpr}, data: ${data}})`;
      }
    }
    if (Object.keys(sendOptions).length > 0) {
      this.variableMap[`${SEND_PREFIX}${this.SEND_COUNTER}`] = this.variableMap[
        `${SEND_PREFIX}${this.SEND_COUNTER}`
      ].concat(`, ${toSource(sendOptions)})`);
    } else {
      this.variableMap[`${SEND_PREFIX}${this.SEND_COUNTER}`] =
        this.variableMap[`${SEND_PREFIX}${this.SEND_COUNTER}`].concat(")");
    }
  ...
  protected addCancel(cancelNode: Node, actionsArray: any[]): void {
    ...
    // Attribute Extraction
    const sendIdAttr = select("./@sendid", cancelNode, true) as
      | Attr
      | undefined;
    const sendIdExprAttr = select("./@sendidexpr", cancelNode, true) as
      | Attr
      | undefined;

    // Event parsing
    if (sendIdAttr?.value !== undefined) {
      if (sendIdExprAttr != null)
        throw Error("Attribute 'sendid' should not occur with 'sendidexpr'");
      idString = sendIdAttr.value;
    } else {
      if (sendIdExprAttr == null)
        throw Error(
          "Attribute 'sendidexpr' must occur with if 'sendid' is not present",
        );
    ...
    if (this.XStateMachineOptions.actions == null)
      this.XStateMachineOptions.actions = {};
    ...
    if (idString !== undefined)
      this.variableMap[
        `${CANCEL_PREFIX}${this.CANCEL_COUNTER}`
      ] = `cancel("${idString}")`;
    else if (idExpr !== undefined)
      this.variableMap[
        `${CANCEL_PREFIX}${this.CANCEL_COUNTER}`
      ] = `cancel((context: any) => (return ${idExpr}))`;
   ...
   * @param actionsArray
   */
  protected async addScript(
    childScriptNode: Node,
    actionsArray: any[],
  ): Promise<void> {
    ...
    if (srcAttr?.value !== undefined)
      scriptString = this.fetch(srcAttr.value) as unknown as string;
    else
      scriptString = select("string(.)", childScriptNode, true) as
        | string
        | undefined;
    ...
    if (scriptString !== undefined) {
      ...
      this.variableMap[
        `${SCRIPT_PREFIX}${this.SCRIPT_COUNTER}`
      ] = `(context: any, event: any {_event}) => ${expressionModded}`;
      this.XStateMachineOptions.actions[
        `${SCRIPT_PREFIX}${this.SCRIPT_COUNTER}`
      ] = `{{${SCRIPT_PREFIX}${this.SCRIPT_COUNTER}}}` as any;
  ...
  protected async addInvoke(
    ...
    const typeExprAttr = select("./@typeExpr", invokeNode, true) as
      | Attr
      | undefined;
    const srcAttr = select("./@src", invokeNode, true) as Attr | undefined;
    const srcExprAttr = select("./@srcexpr", invokeNode, true) as
      | Attr
      | undefined;
    ...
    if (
      typeAttr !== undefined &&
      typeAttr.value !== "http://www.w3.org/TR/scxml/" &&
      typeAttr.value !== "scxml"
    )
      throw new Error("Invoke Type not supported");
    if (typeAttr === undefined && typeExprAttr?.value !== undefined) {
      /* empty */
    }
    if (srcAttr?.value !== undefined) srcName = srcAttr.value;
    if (srcAttr?.value === undefined && srcExprAttr?.value !== undefined) {
      srcName = this.exprParserAndModder(srcExprAttr.value);
    }
    if (idAttr?.value !== undefined) id = idAttr.value;
    if (idAttr?.value === undefined && idlocationAttr?.value !== undefined) {
      idLocation = idlocationAttr.value;
    }
    if (autoForwardAttr?.value !== undefined)
      autoforward = autoForwardAttr.value;
    ...
    for (let i = 0; i < invokeNode.childNodes.length; i++) {
      ...
      switch (childNode.nodeName) {
        ...
        case "content":
          if (srcAttr?.value !== undefined || srcExprAttr?.value !== undefined)
            throw new Error(
              '<invoke> cannot have "src" and <content> at the same time',
            );
    ...
    const childMachineFactoryCode =
      await childMachineParser.generateXStateMachineFactory(scxmlNode);
    ...
    this.variableMap[
      `${MACHINE_SERVICE_PREFIX}${this.MACHINE_SERVICE_COUNTER}`
    ] = `${MACHINE_SERVICE_PREFIX}${this.MACHINE_SERVICE_COUNTER}`;

    /* =============================================== Add Invoke Node ================================================== */

    if (idLocation === undefined) {
      await this.addInvokeNormally(
        currentState,
        initialContext,
        id,
        Boolean(autoforward),
        hasParam ? paramsObj : undefined,
      );
    } else {
      await this.addInvokeAsSpawn(
        currentState,
        initialContext,
        idLocation,
        stateID,
  ...
  protected async addInvokeNormally(
    currentState: any,
    ...
    if (data !== undefined) {
      currentState.invoke.data = this.handleInvokeOrDoneDataParamsObject(
        data,
        initialContext,
      );
    }

    if (this.XStateMachineOptions.services !== undefined)
      this.XStateMachineOptions.services[
        `${MACHINE_SERVICE_PREFIX}${this.MACHINE_SERVICE_COUNTER}`
      ] = `{{${MACHINE_SERVICE_PREFIX}${this.MACHINE_SERVICE_COUNTER}}}` as any;
    currentState.invoke.src = `${MACHINE_SERVICE_PREFIX}${this.MACHINE_SERVICE_COUNTER}`;
    this.MACHINE_SERVICE_COUNTER++;
  }

  protected async addInvokeAsSpawn(
    currentState: any,
    initialContext: Record<string, any>,
    idLocation: string,
    ...
    if (this.XStateMachineConfig.context === undefined)
      this.XStateMachineConfig.context = {};
    if (this.XStateMachineConfig.context._spawnedServices === undefined)
      this.XStateMachineConfig.context._spawnedServices = {};
    if (this.XStateMachineConfig.context._spawnedServicesCounter === undefined)
      this.XStateMachineConfig.context._spawnedServicesCounter = 0;

    let spawnFuncCode = `spawn(InvokedMachineFactory("{{${MACHINE_SERVICE_PREFIX}${this.MACHINE_SERVICE_COUNTER}}}"`;
    if (data !== undefined)
      spawnFuncCode += `, ${this.handleSendorSpawnParamsObject(
        data,
        initialContext,
      )}`;
    spawnFuncCode += ")";
    if (autoforward === true)
      spawnFuncCode += `,{\`${currentStateID}.\${context._spawnedServicesCounter}\`, autoforward: ${String(
        autoforward,
      )}})`;
    else
      spawnFuncCode += `,\`${currentStateID}.\${context._spawnedServicesCounter}\`)`;
    ...
    if (currentState.entry === undefined)
      currentState.entry = `{{${SPAWN_ENTRY_PREFIX}${this.SPAWN_COUNTER}}}`;
    else
      currentState.entry.push(`{{${SPAWN_ENTRY_PREFIX}${this.SPAWN_COUNTER}}}`);

    if (currentState.exit === undefined)
      currentState.exit = `{{${SPAWN_EXIT_PREFIX}${this.SPAWN_COUNTER}}}`;
    else
      currentState.exit.push(`{{${SPAWN_EXIT_PREFIX}${this.SPAWN_COUNTER}}}`);
  ...
  protected addData(dataNode: Node): void {
    ...
    if (idAttr?.value === undefined)
      throw new Error("<data> node does not contain an id");
    ...
    for (let i = 0; i < doneDataNode.childNodes.length; i++) {
      ...
      switch (childNode.nodeName) {
        case "param":
          ...
          if (includesContent)
            throw new Error(
              "<donedata> cannot include both <param> and <content>",
            );
          this.addParam(childNode, paramsObj);
          break;
        ...
        case "content":
          ...
          if (includesContent)
            throw new Error(
              "<donedata> cannot include more than one <content> node",
            );
          includesContent = true;
          // Will parse the first Text Node we see as JSON
  ...
  protected getContentFromNode(
    contentNode: Node,
    parentType: "send" | "invoke" | "donedata" | "WoTSend",
  ): any | Node {
    ...
    if (exprAttr?.value !== undefined) {
      if (parentType === "WoTSend") {
        ...
        return `(context: any, event: any, {_event}) => {return ${this.exprParserAndModder(
          exprAttr.value,
        )}}`;
    } else {
      if (parentType !== "invoke") {
        for (let i = 0; i < contentNode.childNodes.length; i++) {
          ...
          if (
            contentNode.childNodes[i].nodeType ===
            contentNode.childNodes[i].TEXT_NODE
          ) {
        ...
        for (let i = 0; i < contentNode.childNodes.length; i++) {
          ...
          if (
            contentNode.childNodes[i].nodeType ===
            contentNode.childNodes[i].ELEMENT_NODE
          ) {
  ...
  protected addParam(
    paramNode: Node,
    parametersObj: Record<string, any>,
  ): void {
    const nameAttr = select("./@name", paramNode, true) as Attr | undefined;
    const locationAttr = select("./@location", paramNode, true) as
      | Attr
      | undefined;
    const exprAttr = select("./@expr", paramNode, true) as Attr | undefined;

    if (nameAttr?.value === undefined)
      throw new Error('<param> node must have "name" attribute');
    const key = nameAttr.value;

    if (locationAttr?.value !== undefined) {
      if (exprAttr?.value !== undefined)
        throw new Error(
          '<param> node must have either "location" or "expr" attribute',
        );
  ...
  protected handleSendorSpawnParamsObject(
    parametersObj: Record<string, any>,
    initialContext?: Record<string, any>,
  ): string {
    let dataString = "{\n";
    ...
    if (initialContext !== undefined) {
      for (const key in initialContext) {
        if (key in parametersObj) {
          ...
          this.variableMap[`${PARAM_PREFIX}${this.PARAM_COUNTER}`] =
            parametersObj[key].replace("\n,", ",\n");
          this.PARAM_COUNTER++;
        } else {
  ...
  protected handleInvokeOrDoneDataParamsObject(
   ...
   * @returns
   */
  protected conditionParser(expression: string): {
    type?: string;
    expr?: string;
    in?: string;
  } {
    ...
    AbstractSyntaxTree.walk(conditionTree, (node: any, parent: any) => {
      if (
        ...
        if (parent.type === "VariableDeclarator")
          declaredVariable.push(node.name);

        if (
          parent.type === "MemberExpression" &&
          (!(AbstractSyntaxTree.match(parent.property, node) as boolean) ||
          ...
          (parent.type !== "CallExpression" ||
            !(AbstractSyntaxTree.match(parent.callee, node) as boolean)) &&
          parent.type !== "VariableDeclarator" &&
          node.name !== "_event" &&
          node.name !== "event" &&
          node.name !== "undefined" &&
          !declaredVariable.includes(node.name)
        ) {
          node.name = `context["${node.name as string}"]`;
        }
      }
      ...
      if (
        ...
        switch (parent.type) {
          ...
          case "LogicalExpression": {
            ...
            let leftOrRight = "";
            if (AbstractSyntaxTree.match(parent.right, node) as boolean)
              leftOrRight = "right";
            ...
            AbstractSyntaxTree.walk(
              conditionTree,
              (parentNode: any, parentOfParentNode: any) => {
                if (AbstractSyntaxTree.match(parent, parentNode) as boolean)
                  parentOfParent = parentOfParentNode;
              },
            );
            ...
            if (
              ...
              parentOfParent.type !== "ExpressionStatement"
            )
              throw Error(
                "In predicate must be on the top level of an AND-logical statement",
              );
          ...
          default:
            throw Error(
              "Conditions can only contain expression statements or logical expressions",
            );
        }
      }
    });
    ...
    if (!noCondition) {
      ...
      if (this.XStateMachineOptions.guards === undefined)
        this.XStateMachineOptions.guards = {};
      this.variableMap[
        `${COND_PREFIX}${this.COND_COUNTER}`
      ] = `(context: any, event: any, { cond, _event }) => ${condition}`;
      this.XStateMachineOptions.guards[`${COND_PREFIX}${this.COND_COUNTER}`] =
        `{{${COND_PREFIX}${this.COND_COUNTER}}}` as any;
  ...
  protected exprParserAndModder(expression: string): string {
    ...
    const isObjectRegex = /[{|,]\s*("\w+":\s*[a-zA-Z_$]\w+)/gm;
  ...
  protected findParentStates(stateNode: Node): string[] {
    ...
    while (currentParentNode !== null) {
      if (
        currentParentNode.nodeName === "state" ||
        currentParentNode.nodeName === "parallel"
      ) {
