...
export class SCXML2WoTCodeParser extends SCXML2XStateCodeParser {
  ...
  public async generateXStateCode(
    scxml: string | Node,
    ...
    if (typeof scxml === "string") {
      scxmlDOM = new DOMParser().parseFromString(
        scxml,
        "application/scxml+xml",
      );
      scxmlNode = select("//scxml:scxml", scxmlDOM, true) as Node;
      wotAffrNode = select("//wot:affordances", scxmlDOM, true) as
        | Node
        | undefined;
      // Otherwise Node is ready
    } else {
      if (scxml.nodeName !== "scxml")
        throw new Error("Can only parse SCXML node");
      scxmlNode = scxml;
      scxmlDOM = scxmlNode.ownerDocument as Document;
      wotAffrNode = select("//wot:affordances", scxmlNode, true) as
        | Node
        | undefined;
    }
    ...
    const needPropertyWriteHandler = this.addPropertyWriteHandlerState(
      scxmlDOM,
      writableProperties,
    );
    if (needPropertyWriteHandler) {
      this.codeSnippets.properties.writeproperty =
        this.codeSnippets.properties.writeproperty.concat("");
    }

    this.preproccesModelNodes(scxmlDOM);
    const dataModelElement = select(
      "//scxml:datamodel",
      scxmlDOM,
      true,
    ) as Element;
    const stepSizeDataElement = scxmlDOM.createElementNS(
...
const service = interpret(${
      machineID ?? this.XStateMachineConfig.id ?? "machine"
    })
...
`;
  ...
  protected addSendWOTResponse(sendNode: Node, actionsArray: any[]): void {
    ...
    const eventExprAttr = select("./@eventexpr", sendNode, true) as
      | Attr
      | undefined;
    const targetAttr = select("./@target", sendNode, true) as Attr | undefined;
    const targetExprAttr = select("./@targetexpr", sendNode, true) as
      | Attr
      | undefined;
    // const typeAttr = select('./@type', sendNode, true) as Attr | undefined
    // const typeExprAttr = select('./@typeExpr', sendNode, true) as Attr | undefined
    ...
    if (eventAttr?.value !== undefined) {
      if (eventExprAttr != null)
        throw Error("Attribute 'eventExpr' should not occur with 'event'");
      eventString = eventAttr.value;
    } else {
      if (eventExprAttr == null)
        throw Error(
          "Attribute 'eventExpr' must occur with if 'event' is not present",
        );
    ...
    if (targetString !== undefined && targetString !== "Consumer")
      throw new Error(
        "<send> of type WoTIOProcessor can only have Consumer as target. Current target is " +
          targetString,
      );
    ...
    if (this.XStateMachineOptions.actions === undefined)
      this.XStateMachineOptions.actions = {};

    if (namelistAttr?.value !== undefined)
      this.handleNameList(namelistAttr.value, paramsObj);
    // Handle <content> and <param> Children
    for (let i = 0; i < sendNode.childNodes.length; i++) {
      ...
      switch (childNode.nodeName) {
        case "param":
          includesParam = true;
          if (includesContent)
            throw new Error("<send> cannot include both <param> and <content>");
        ...
        case "content":
          ...
          if (includesContent)
            throw new Error(
              "<send> cannot include more than one <content> node",
            );
    ...
    if (includesParam) {
      this.variableMap[`${PARAM_OBJECT_PREFIX}${this.PARAM_OBJECT_COUNTER}`] =
        this.handleSendorSpawnParamsObject(paramsObj);
    ...
    if (eventString !== undefined && data === undefined) {
      if (delayNumber !== undefined)
        code += ` setTimeout(()=>{ responseHandlerEmitter.emit("${eventString}") }, ${delayNumber}) }`;
      else if (delayExpr !== undefined)
        code += ` setTimeout(()=>{ responseHandlerEmitter.emit("${eventString}") }, ${delayExpr}) }`;
      else code += ` responseHandlerEmitter.emit("${eventString}") }`;
    } else if (eventString !== undefined && data !== undefined) {
      if (delayNumber !== undefined)
        code += ` setTimeout(()=>{ responseHandlerEmitter.emit("${eventString}", ${data}) }, ${delayNumber}) }`;
      else if (delayExpr !== undefined)
        code += ` setTimeout(()=>{ responseHandlerEmitter.emit("${eventString}", ${data}) }, ${delayExpr}) }`;
      else code += ` responseHandlerEmitter.emit("${eventString}", ${data}) }`;
    } else if (eventExpr !== undefined && data === undefined) {
      if (delayNumber !== undefined)
        code += ` setTimeout(()=>{ responseHandlerEmitter.emit("${eventExpr}") }, ${delayNumber}) }`;
      else if (delayExpr !== undefined)
        code += ` setTimeout(()=>{ responseHandlerEmitter.emit("${eventExpr}") }, ${delayExpr}) }`;
      else code += ` responseHandlerEmitter.emit("${eventExpr}") }`;
    } else if (eventExpr !== undefined && data !== undefined) {
      if (delayNumber !== undefined)
        code += ` setTimeout(()=>{ responseHandlerEmitter.emit("${eventExpr}", ${data}) }, ${delayNumber}) }`;
      else if (delayExpr !== undefined)
        code += ` setTimeout(()=>{ responseHandlerEmitter.emit("${eventExpr}", ${data}) }, ${delayExpr}) }`;
    ...
    if (this.XStateMachineOptions.actions === undefined)
      this.XStateMachineOptions.actions = {};
    this.XStateMachineOptions.actions[
      `${WOT_RESPONSE_PREFIX}${this.WOT_RESPONSE_COUNTER}`
    ] = `{{${SEND_PREFIX}${this.SEND_COUNTER}}}` as any;
  ...
  protected addSendSim(sendNode: Node, actionsArray: any[]): void {
    ...
    if (targetString !== undefined && !targetString.startsWith("ws://"))
      throw new Error(
        "<send> of type WoTSimProcessor can only have Websocket servers as target. Current target is " +
          targetString,
      );
    ...
    if (data !== undefined && targetString !== undefined) {
      if (delayNumber !== undefined)
        code += ` setTimeout(()=>{ ws${this.simulationTargets.indexOf(
          targetString,
        )}.send(JSON.stringify(${data})) }, ${delayNumber}) }`;
      else if (delayExpr !== undefined)
        code += ` setTimeout(()=>{ ws${this.simulationTargets.indexOf(
          targetString,
        )}.send(JSON.stringify(${data})) }, ${delayExpr}) }`;
      else
        code += ` ws${this.simulationTargets.indexOf(
          targetString,
    ...
    if (this.XStateMachineOptions.actions === undefined)
      this.XStateMachineOptions.actions = {};
    this.XStateMachineOptions.actions[
      `${WOT_SIM_PREFIX}${this.WOT_SIM_COUNTER}`
    ] = `{{${SEND_PREFIX}${this.SEND_COUNTER}}}` as any;
  ...
  protected async addTransition(
    ...
    const eventAttr = select("./@event", transitionNode, true) as
      | Attr
      | undefined;
    const condAttr = select("./@cond", transitionNode, true) as
      | Attr
      | undefined;
    ...
    if (eventAttr?.value !== undefined) {
      ...
      const xpath =
        parentId !== null
          ? `//wot:property[@stateElement="${parentId}"]`
          : undefined;
      const statePropertyNode =
        xpath !== undefined ? select(xpath, this.scxmlNode) : null;
      const stateProperty =
        statePropertyNode !== null && xpath !== undefined
          ? ((select(xpath, this.scxmlNode) as Node[])[0] as
              | Element
              | undefined)
          : undefined;
      const stateIsProperty = stateProperty !== undefined;
      const statePropertyName = stateIsProperty
        ? stateProperty.getAttribute("name")
        : null;
      ...
      if (stateIsProperty && internalOp !== undefined) {
        ...
        const affordancesObject = this.protoTD[
          affordancesMap[internalOp.affordanceType]
        ] as Record<string, any>;
        affordancesObject[internalOp.name] ??= {};
        affordancesObject[internalOp.name][
          `scxml:${internalOp.affordanceType}`
        ] ??= {};
        affordancesObject[internalOp.name][
          `scxml:${internalOp.affordanceType}`
        ][internalOp.op] ??= {};
        ...
        affordancesObject[internalOp.name][
          `scxml:${internalOp.affordanceType}`
        ][internalOp.op].affects ??= [];
        if (
          statePropertyName !== null &&
          !(
            affordancesObject[internalOp.name][
              `scxml:${internalOp.affordanceType}`
            ][internalOp.op].affects as string[]
          ).includes(statePropertyName)
        )
          affordancesObject[internalOp.name][
            `scxml:${internalOp.affordanceType}`
          ][internalOp.op].affects.push(statePropertyName);
        // AvailableInState keyword
        affordancesObject[internalOp.name][
          `scxml:${internalOp.affordanceType}`
        ][internalOp.op].availableInState ??= [];
        if (id !== undefined && statePropertyName !== null) {
          if (
            !(
              affordancesObject[internalOp.name][
                `scxml:${internalOp.affordanceType}`
              ][internalOp.op].availableInState as string[]
            ).includes(`${statePropertyName}.${id}`)
          )
            affordancesObject[internalOp.name][
              `scxml:${internalOp.affordanceType}`
            ][internalOp.op].availableInState.push(
              `${statePropertyName}.${id}`,
            );

          if (!internalOp.availableIn.includes(parentsString.concat(`.${id}`)))
            internalOp.availableIn.push(parentsString.concat(`.${id}`));
          ...
          if (cond !== undefined) {
            ...
            affordancesObject[internalOp.name][
              `scxml:${internalOp.affordanceType}`
            ][internalOp.op][`${statePropertyName}.${id}`] ??= [];
            affordancesObject[internalOp.name][
              `scxml:${internalOp.affordanceType}`
            ][internalOp.op][`${statePropertyName}.${id}`].push({ cond });
      ...
      for (let i = 0; i < assignArray.length; i++) {
        ...
        const xpath =
          location !== null
            ? `//wot:property[@dataElement="${location}"]`
            : undefined;
        const statePropertyNode =
          xpath !== undefined && !(location?.startsWith("event") ?? false)
            ? select(xpath, this.scxmlNode)
            : null;
        const dataProperty =
          statePropertyNode !== null && xpath !== undefined
            ? ((select(xpath, this.scxmlNode) as Node[])[0] as
        ...
        const dataPropertyName = dataIsProperty
          ? dataProperty.getAttribute("name")
          : null;
        ...
        if (dataIsProperty && internalOp !== undefined) {
          ...
          if (
            ...
            ).includes(dataPropertyName)
          )
            affordancesObject[internalOp.name][
              `scxml:${internalOp.affordanceType}`
            ][internalOp.op].affects.push(dataPropertyName);
        }
      }
    }
  }

  protected async addNormalState(
    childStateNode: Node,
    parentNodeStatesObj: any,
  ): Promise<void> {
    await super.addNormalState(childStateNode, parentNodeStatesObj);
    const idAttr = select("./@id", childStateNode, true) as Attr;
    const wotPropertyAttr = select("./@wot:property", childStateNode, true) as
      | Attr
      | undefined;
    ...
    if (wotPropertyAttr !== undefined) {
      ...
      // Add read handler
      if (
        this.protoTD.properties[id]?.writeOnly === false ||
        this.protoTD.properties[id]?.writeOnly === undefined
      ) {
  ...
  protected async addParallelState(
    ...
    if (wotPropertyAttr !== undefined) {
      ...
      this.getStateSchema(
        childParallelNode,
        this.protoTD.properties[id] as any,
      );
  ...
  protected addWoTProperty(wotPropertyElement: Element): void {
    ...
    const dataElementAttr = select(
      "./@dataElement",
      wotPropertyElement,
      true,
    ) as Attr | undefined;
    const accessPropsAttr = select(
      "./@accessProps",
      wotPropertyElement,
      true,
    ) as Attr | undefined;
    const stateElementAttr = select(
      "./@stateElement",
      wotPropertyElement,
      true,
    ) as Attr | undefined;
    ...
    const accessProps =
      accessPropsAttr?.value !== undefined
        ? (JSON.parse(accessPropsAttr.value) as boolean)
        : undefined;

    // add property to TD
    if (this.protoTD.properties === undefined) this.protoTD.properties = {};
    if (this.protoTD.properties[name] === undefined)
      this.protoTD.properties[name] = {};

    if (
      dataElementID !== undefined &&
      (accessProps === false || accessProps === undefined)
    ) {
      let dataElement;
      ...
      if (this.scxmlNode !== undefined) {
        dataElement = (
          select(
            `//scxml:data[@id="${dataElementID}"]`,
            this.scxmlNode,
          ) as Node[]
        )[0] as Element | undefined;
        if (dataElement === undefined)
          throw new Error(
            `Cannot find <data> element with id: ${dataElementID} for property ${name}`,
          );

        const contentElement = select(
          "./scxml:content",
          wotPropertyElement,
          true,
        ) as Element | undefined;
        if (contentElement === undefined || contentElement.textContent === null)
          throw new Error(
            `Cannot handle property ${name} without valid <content>`,
          );
      ...
      const childStateNode = (
        select(
          `//scxml:state[@id="${stateElementID}"] | //scxml:parallel[@id="${stateElementID}"] | //scxml:final[@id="${stateElementID}"]`,
          this.scxmlNode,
        ) as Node[]
      )[0] as Element | undefined;
      if (childStateNode === undefined)
        throw new Error(
          `Cannot find state element with id: ${stateElementID} for property ${name}`,
        );
      const contentElement = select(
        "./scxml:content",
        wotPropertyElement,
      ...
      const propertyObj =
        contentElement?.textContent != null
          ? JSON.parse(contentElement.textContent)
          : undefined;
      this.protoTD.properties[name] = propertyObj;
      this.addPropertyState(
        name,
        stateElementID,
        wotPropertyElement,
        childStateNode,
      ...
      throw new Error(
        `Property "${name}" must define either "dataElement" or "stateElement"`,
      );
    }
  }
  ...
  protected addWoTAction(wotActionElement: Element): void {
    ...
    if (this.protoTD.actions[name] === undefined)
      this.protoTD.actions[name] = {};
    const contentElement = select("./scxml:content", wotActionElement, true) as
      | Element
      | undefined;
    if (contentElement === undefined || contentElement.textContent === null)
      throw new Error("Cannot handle action without valid <content>");
  ...
  protected addWoTEvent(wotEventElement: Element): void {
    ...
    const name = nameAttr.value;
    const emitEvent = (select("./@emitEvent", wotEventElement, true) as Attr)
      ?.value;

    if (emitEvent === undefined)
      throw new Error(`emitEvent of Event ${name} is not defined!`);
    ...
    const contentElement = select("./scxml:content", wotEventElement, true) as
      | Element
      | undefined;
    if (contentElement === undefined || contentElement.textContent === null)
      throw new Error("Cannot handle action without valid <content>");
  ...
  protected addPropertyData(
    ...
    const hasEmitEvent =
      (select("./@emitEvent", propertyNode, true) as Attr | undefined)
        ?.value !== undefined;
    if (contentElement === undefined || contentElement.textContent === null)
      throw new Error(
        `Cannot handle property ${propertyName} without valid <content>`,
      );
    ...
    if (propertySchema === undefined || propertySchema === null)
      throw new Error(
        `<wot:property> ${propertyName} with "dataElement" does not contain a <schema>`,
      );

    if (this.protoTD.properties === undefined) this.protoTD.properties = {};
    if (this.protoTD.properties[propertyName] === undefined)
      this.protoTD.properties[propertyName] = propertySchema;

    const readable =
      this.protoTD.properties[propertyName]?.writeOnly === undefined ||
      this.protoTD.properties[propertyName]?.writeOnly === false;
    const writable =
      this.protoTD.properties[propertyName]?.readOnly === undefined ||
      this.protoTD.properties[propertyName]?.readOnly === false;
    const observable =
      this.protoTD.properties[propertyName]?.observable === true;

    // find read prop
    const readPropNode = select(
      './wot:op[@type="readproperty"]',
      propertyNode,
      true,
    ) as Element | undefined;
    let readPropStateDependentAttr,
      readPropResponseEventAttr,
      readPropEventAttr;
    if (readPropNode !== undefined) {
      // Event to initiate property read
      readPropEventAttr = select("./@event", readPropNode, true) as
        | Attr
        | undefined;
      // Is reading the property dependent on current state
      readPropStateDependentAttr = select(
        "./@stateDependent",
        readPropNode,
        true,
      ) as Attr | undefined;
      // Event that is emitted from the state machine once the read operation is fulfilled
      readPropResponseEventAttr = select(
        "./@responseEvent",
        readPropNode,
        true,
      ) as Attr | undefined;
    ...
    const readPropStateDependent =
      readPropStateDependentAttr?.value !== undefined &&
      readPropStateDependentAttr?.value === "true";
    const readHasResponseEvent = readPropResponseEventAttr?.value !== undefined;
    if (!hasReadPropEvent && readHasResponseEvent)
      console.warn(
        `readproperty ${propertyName} has a response event defined, but no event, response event will be ignored!`,
      );

    // find write prop
    const writePropNode = select(
      './wot:op[@type="writeproperty"]',
      propertyNode,
      true,
    ) as Element | undefined;
    let writePropStateDependentAttr,
      writePropResponseEventAttr,
      writePropEventAttr;
    if (writePropNode !== undefined) {
      writePropEventAttr = select("./@event", writePropNode, true) as
        | Attr
        | undefined;
      writePropStateDependentAttr = select(
        "./@stateDependent",
        writePropNode,
        true,
      ) as Attr | undefined;
      writePropResponseEventAttr = select(
        "./@responseEvent",
        writePropNode,
        true,
      ) as Attr | undefined;
    ...
    const writeHasResponseEvent =
      writePropResponseEventAttr?.value !== undefined;
    const writePropStateDependent =
      writePropStateDependentAttr?.value !== undefined &&
      writePropStateDependentAttr?.value === "true";
    if (!hasWritePropEvent && writeHasResponseEvent)
      console.warn(
        `Writeproperty ${propertyName} has a response event defined, but no event, response event will be ignored!`,
      );

    // add read handler if no <wot:op> or if not state dependent and no event (always possible to read and response is instant)
    if (
      readable &&
      (readPropNode === undefined ||
        (!readPropStateDependent && !hasReadPropEvent))
    ) {
    ...
    if (
      writable &&
      (writePropNode === undefined ||
        (!writePropStateDependent && !hasWritePropEvent))
    ) {
  ...
  protected addPropertyDataNamelist(
    propertyName: string,
    dataName: string,
    propertyNode: Node,
  ): void {
    if (this.scxmlNode === undefined) throw Error("SCXML undefined");
    // Extract Schema
    const contentElement = select("./scxml:content", propertyNode, true) as
      | Element
      | undefined
      | null;
    ...
    if (
      ...
      console.log(
        `Warning: found a <content> with schema inside a namelisted <property> ${propertyName}. Will merge with generated schema`,
      );
    ...
    if (
      contentElement?.textContent !== null &&
      contentElement?.textContent !== undefined
    ) {
      propertySchema = JSON.parse(contentElement.textContent);
      if (propertySchema.uriVariables !== undefined)
        accessParameter = Object.keys(propertySchema.uriVariables)[0];
    }

    // Find data element
    const dataElement = (
      select(`//scxml:data[@id="${dataName}"]`, this.scxmlNode) as Node[]
    )[0];
    ...
    if (exprAttr?.value !== undefined) expr = exprAttr.value;
      ...
      for (let i = 0; i < dataElement.childNodes.length; i++) {
        ...
        if (
          ...
          expr = childNode.nodeValue;
      }
    }

    if (expr === undefined) {
      throw new Error(
        `Namelisted <property> ${propertyName} is linked to <data> ${dataName} that does not contain any experssion of data`,
      );
    } else {
      ...
      if (typeof dataJSON !== "object")
        throw new Error(
          `Namelisted <property> ${propertyName} is linked to <data> ${dataName} that is not an object`,
        );
    }

    // Generate schema
    accessParameter ??= (
      select("./@accessParamter", propertyNode, true) as Attr | undefined
    )?.value;
    if (accessParameter === undefined || accessParameter === null)
      throw Error(
        `Cannot handle namelisted <property> ${propertyName} with "accesProps" and no "accessParameter"`,
      );
    propertySchema ??= {};
    ...
    if (propertySchema.type === undefined) {
      for (const prop in dataJSON) {
        ...
        if (propertySchema.type === undefined) {
          ...
          dataJsonType !== propertySchemaType
        ) {
  ...
  protected addPropertyState(
    ...
    this.protoTD.properties[propertyName] ??= {};
    if (
      this.protoTD.properties === undefined ||
      this.protoTD.properties[propertyName] === undefined
    ) {
      throw new Error("Unexpected error");
    }
    (
      this.protoTD.properties[propertyName] as Record<string, any>
    ).title ??= `${stateId}`;
    (
      this.protoTD.properties[propertyName] as Record<string, any>
    ).description ??= `Exposed State ${stateId}`;
    this.getStateSchema(
      stateNode,
      this.protoTD.properties[propertyName] as any,
    );
  ...
  protected findStateLevel(stateNode: Node): number {
    ...
    for (i = 0; i < stateNode.childNodes.length; i++) {
      ...
      if (
        ...
        (childNode.nodeName === "state" ||
          childNode.nodeName === "parallel" ||
          childNode.nodeName === "final" ||
          childNode.nodeName === "initial" ||
          childNode.nodeName === "history")
      ) {
    ...
    if (stateLevel === 0) {
      ...
      for (let i = 0; i < stateNode.childNodes.length; i++) {
        ...
        if (
          ...
          if (idAttr?.value !== undefined && nodeType === "parallel")
            stateValues[idAttr.value] = {};
  ...
  protected getStateSchema(
    ...
    if (typeof possibilities === "string") {
      ...
      possibilities.every((value) => typeof value === "string")
    ) {
      ...
      for (const possibility of possibilities) {
        ...
        if (typeof possibility === "object" && !Array.isArray(possibility))
          complexChildren.push(possibility);
      }
      ...
      for (const complexStatePossibility of complexChildren) {
        ...
        statePropetySchema.required.push(
          ...Object.keys(complexStatePossibility),
        );
        for (const state in complexStatePossibility) {
          const childStateNode = (
            select(
              `//scxml:state[@id="${state}"] | //scxml:parallel[@id="${state}"] | //scxml:final[@id="${state}"]`,
              stateNode,
            ) as Node[]
          )[0] as Node | undefined;
  ...
  protected addPropertyWriteHandlerState(
    scxml: Node,
    properties?: string[],
  ): boolean {
    const scxmlDoc =
      scxml.ownerDocument !== null ? scxml.ownerDocument : (scxml as Document);
    ...
    if (
      scxmlElementChildren.length === 1 &&
      scxmlElementChildren[0].nodeName === "parallel"
    )
      isTopLevelParallel = true;

    // create State
    const propertyWriteHandlerNode = scxmlDoc.createElementNS(
      "http://www.w3.org/2005/07/scxml",
      "state",
    );
  ...
  protected addParallelWotWrapper(scxml: Node): Element {
    ...
    let isTopLevelParallel = false;
    const wotWrapper: Element | undefined = select(
      './scxml:parallel[@id="_wotwrapper"]',
      scxmlNode,
      true,
    ) as Element;
    ...
    if (
      (scxmlElementChildren.length === 1 &&
        scxmlElementChildren[0].nodeName === "parallel") ||
      wotWrapper !== undefined
    )
      isTopLevelParallel = true;

    if (!isTopLevelParallel) {
      const wrapperNode = scxmlDoc.createElementNS(
        "http://www.w3.org/2005/07/scxml",
        "parallel",
      );
      wrapperNode.setAttribute("id", "_wotwrapper");
      const machineWrapperNode = scxmlDoc.createElementNS(
        "http://www.w3.org/2005/07/scxml",
        "state",
      );
      ...
      for (let i = 0; i < scxmlChildren.length; i++) {
        ...
        if (
          ...
          stateNodeTypes.includes(child.nodeName)
        )
          stateChildren.push(child);
      }
      for (const child of stateChildren) {
        machineWrapperNode.appendChild(child);
      }
  ...
  protected findWritableDataProperties(scxmlNode: Node): string[] | undefined {
    ...
    const wotPropertyElements = select(
      "//wot:property[@dataElement]",
      scxmlNode,
    ) as Element[];
    for (const wotPropertyElement of wotPropertyElements) {
      ...
      const propertyName = (select("./@name", wotPropertyElement, true) as Attr)
        .value;
      const dataElementId = (
        select("./@dataElement", wotPropertyElement, true) as Attr
      ).value;
      ...
      const writable =
        schemaObj?.readOnly === undefined || schemaObj?.readOnly === false;
      ...
      if (
        writable &&
        (writePropNode === undefined ||
          (!writePropStateDependent && !writeHasResponseEvent))
      ) {
  ...
  protected addOp(
    ...
    affordanceObject: Record<string, any>,
  ): void {
    const affordanceElement = opElement.parentNode as Element;

    let dataElement: string | null,
      stateElement: string | null,
      availableInAttr: string | null,
      propertyElementId: string | undefined,
      propertyType: "data" | "state" | undefined;

    const typeAttr = select("./@type", opElement, true) as Attr;
    const eventAttr = select("./@event", opElement, true) as Attr | undefined;
    const responseEventAttr = select("./@responseEvent", opElement, true) as
      | Attr
      | undefined;
    const stateDependentAttr = select("./@stateDependent", opElement, true) as
      | Attr
      | undefined;
    ...
    const stateDependent =
      stateDependentAttr?.value !== undefined &&
      stateDependentAttr?.value === "true";

    if (affordanceObject[`scxml:${affordanceType}`] === undefined)
      affordanceObject[`scxml:${affordanceType}`] = {};
    ...
    if (event === undefined && stateDependent) {
      ...
      for (let i = 0; i < availableIn.length; i++) {
        ...
        const stateElement = select(
          `//scxml:state[@id="${availableIn[i]}"] | //scxml:parallel[@id="${availableIn[i]}"] | //scxml:final[@id="${availableIn[i]}"]`,
          this.scxmlNode,
        ) as Element[];
        ...
        if (stateIsProperty && statePropertyName !== null) {
          ...
          affordanceObject[`scxml:${affordanceType}`][
            type
          ].availableInState.push(`${statePropertyName}.${availableIn[i]}`);
        }
      }
    }
    ...
    if (affordanceElement.tagName === "wot:action") {
      affordanceObject.synchronous = !(
        responseEvent === undefined || stateDependent
      );
    }

    this.internalOps.push({
      name,
      affordanceType,
      affordanceObject,
      event,
    ...
    for (const op of this.internalOps) {
      switch (op.affordanceType) {
        case "property":
          switch (op.op) {
            case "readproperty":
              if (
                op.event === undefined &&
                op.stateDependent !== undefined &&
                op.stateDependent
              ) {
                // Define property handler
                this.codeSnippets.properties[
                  op.op
                ] += `thing.setPropertyReadHandler('${op.name}', async (inputData, options) => {\n`;
                ...
                if (op.availableIn.length > 0)
                  this.codeSnippets.properties[op.op] +=
                    this.generateStateCheckCode(op);
                // If property is a data Element, return data from context
                if (
                  op.propertyType !== undefined &&
                  op.propertyType === "data" &&
                  op.propertyElementId !== undefined
                ) {
                  this.codeSnippets.properties[
                    op.op
                  ...
                  const stateNode = (
                    select(
                      `//scxml:state[@id="${op.propertyElementId}"] | //scxml:parallel[@id="${op.propertyElementId}"] | //scxml:final[@id="${op.propertyElementId}"]`,
                      this.scxmlNode,
                    ) as Node[]
                  )[0] as Element;
                  let parents = this.findParentStates(stateNode);
                ...
                if (op.responseEvent === undefined)
                  throw new Error(
                    `readproperty ${op.name} requires a response event with data!`,
                  );
            ...
            case "writeproperty":
              this.codeSnippets.properties[
                op.op
              ] += `thing.setPropertyWriteHandler('${op.name}', async (inputData, options) => {`;
              if (
                op.event === undefined &&
                op.stateDependent !== undefined &&
                op.stateDependent &&
            ...
            case "observeproperty":
              if (op.event === undefined)
                throw new Error(
                  `${op.op} ${op.name} must have an "event" attribute`,
                );
              if (op.responseEvent !== undefined) {
                this.codeSnippets.properties[
                  op.op
                ] += `thing.setPropertyObserveHandler('${op.name}', async (inputData, options) => {
                  const responsePromise = once(responseHandlerEmitter, "${op.responseEvent}")
                  service.send({type: '${op.event}', data: { payload: await inputData.value(), uriVariables: options?.uriVariables }} as any)
                  const [data] = await responsePromise
                  return data
            ...
            case "unobserveproperty":
              ...
              if (op.responseEvent !== undefined) {
                this.codeSnippets.properties[
                  op.op
                ] += `thing.setPropertyUnobserveHandler('${op.name}', async (inputData, options) => {
                  const responsePromise = once(responseHandlerEmitter, "${op.responseEvent}")
                  service.send({type: '${op.event}', data: { payload: await inputData.value(), uriVariables: options?.uriVariables }} as any)
                  const [data] = await responsePromise
                  return data
        ...
        case "action":
          ...
          switch (op.op) {
            case "invokeaction":
              this.codeSnippets.actions[
                op.op
              ] += `thing.setActionHandler('${op.name}', async (inputData, options) => {`;
              if (op.availableIn.length > 0)
                this.codeSnippets.actions[op.op] +=
                  this.generateStateCheckCode(op);
              if (
                op.affordanceObject.synchronous !== undefined &&
                op.affordanceObject.synchronous === false
              ) {
                this.codeSnippets.actions[
                  op.op
                ...
                `;
                if (
                  op.affordanceObject?.data !== undefined ||
                  op.affordanceObject?.uriVariables !== undefined
                ) {
                  this.codeSnippets.actions[op.op] += ", data: { ";
                  if (op.affordanceObject?.data !== undefined)
                    this.codeSnippets.actions[op.op] +=
                      "payload: await inputData.value(),";
                  if (op.affordanceObject?.uriVariables !== undefined)
                    this.codeSnippets.actions[op.op] +=
                      "uriVariables: options?.uriVariables,";
                  this.codeSnippets.actions[op.op] += "}";
                }
                ...
                if (op.responseEvent === undefined)
                  throw new Error(
                    `invokeaction ${op.name} is sync but no response event was found!`,
                  );
                this.codeSnippets.actions[op.op] += `
                const responsePromise = once(responseHandlerEmitter, "${op.responseEvent}")
                service.send({type: '${op.event}'
        ...
        case "event":
          ...
          switch (op.op) {
            case "subscribeevent":
              if (op.responseEvent !== undefined) {
                this.codeSnippets.events[
                  op.op
                ] += `thing.setEventSubscribeHandler('${op.name}', async (inputData, options) => {
                  const responsePromise = once(responseHandlerEmitter, "${op.responseEvent}")
                  service.send({type: '${op.event}', data: { payload: await inputData.value(), uriVariables: options?.uriVariables }} as any)
                  const [data] = await responsePromise
                  return data
            ...
            case "unsubscribeevent":
              if (op.responseEvent !== undefined) {
                this.codeSnippets.events[
                  op.op
                ] += `thing.setEventUnsubscribeHandler('${op.name}', async (inputData, options) => {
                  const responsePromise = once(responseHandlerEmitter, "${op.responseEvent}")
                  service.send({type: '${op.event}', data: { payload: await inputData.value(), uriVariables: options?.uriVariables }} as any)
                  const [data] = await responsePromise
                  return data
  ...
  protected generateStateCheckCode(
    op: InternalOperationRepresentation,
  ): string {
    let resultString = "";
    resultString += "const currentState = service.getSnapshot()\n";
    const absoluteStates = op.availableIn.map((state) => `"${state}"`);
    resultString += `if(![${absoluteStates.toString()}].some((s) => currentState.matches(s))) {
        throw new Error("${op.op} ${
      op.name
    } is not accessible in current state")
      } else {`;
    return resultString;
  }
  ...
  protected addMachineStateSchemaAndHandler(): void {
    ...
    this.codeSnippets.properties.readproperty +=
      'thing.setPropertyReadHandler("state", async () => service.getSnapshot().toJSON())\n';
  }
  ...
  protected preproccesModelNodes(scxml: Node): void {
    ...
    for (const modelElement of modelElements) {
      ...
      if (modelServer == null)
        throw new Error('Model does not specify simulation server "server"');
      modelServer = modelServer.trim();
      if (!modelServer.startsWith("ws")) modelServer = "ws://" + modelServer;
      if (!this.simulationTargets.includes(modelServer))
        this.simulationTargets.push(modelServer);

      // get <onentry>
      const onentryNode = modelElement.parentNode as Element;
      if (onentryNode?.tagName !== "onentry")
        throw new Error("Model immediate parent must be <onentry>");
      // get <state> or <parallel>
      const stateNode = onentryNode?.parentNode as Element;
      if (stateNode?.tagName !== "state" && stateNode?.tagName !== "parallel")
        throw new Error("Model second parent must be <state> or <parallel>");
      const stateNodeId = stateNode.getAttribute("id");
      if (stateNodeId === null)
        throw new Error("Model second parent must have an id");
      ...
      for (let i = 0; i < modelElement.childNodes.length; i++) {
        ...
        if (
          child.nodeType === child.ELEMENT_NODE &&
          (child as Element).tagName === "content"
        )
          modelContent = child;

        if (
          child.nodeType === child.ELEMENT_NODE &&
          (child as Element).tagName === "variable"
        )
          variableElements.push(child as Element);
      }
      if (modelContent === null || modelContent === undefined)
        throw new Error(
          `Model '${modelId}' must have <content> with modelica model as string`,
        );
      let modelicaString = modelContent.textContent;
      if (modelicaString === null || modelicaString === undefined)
        throw new Error(
          `Model '${modelId}' must have <content> with modelica model as string`,
        );
      ...
      if (!fs.existsSync("models") || !fs.lstatSync("models").isDirectory())
        fs.mkdirSync(path.join("models"));
      ...
      if (regArray === null)
        throw new Error(
          `Model '${modelId}' does not have an ASCII name in string`,
        );
      const modelName = regArray[1];

      // Get Parameters
      const parametersRegex =
        /parameter\s+(?:Real|Integer|String|Enum)\s+(\w+)/gm;
      let matches = modelicaString.matchAll(parametersRegex);
      for (const match of matches) {
        (modelVariables.parameters as Record<string, string | undefined>)[
          match[1]
        ] = undefined;
      }

      // Get Inputs
      const inputsRegex = /input\s+(?:Real|Integer|String|Enum)\s+(\w+)/gm;
      matches = modelicaString.matchAll(inputsRegex);
      for (const match of matches) {
        (modelVariables.inputs as Record<string, string | undefined>)[
          match[1]
        ] = undefined;
      }

      // Get Outputs
      const outputsRegex = /output\s+(?:Real|Integer|String|Enum)\s+(\w+)/gm;
      matches = modelicaString.matchAll(outputsRegex);
      for (const match of matches) {
        (modelVariables.outputs as Record<string, string | undefined>)[
          match[1]
        ] = undefined;
      }
      ...
      for (let i = 0; i < variableElements.length; i++) {
        ...
        if (variableName === null)
          throw new Error(`Variable in model ${modelId} does not have a name`);

        const variableLocation = variable.getAttribute("location");
        if (variableLocation === null)
          throw new Error(
            `Variable in model ${modelId} does not have a Location`,
          );

        if (Object.keys(modelVariables.parameters).includes(variableName))
          (modelVariables.parameters as Record<string, string | undefined>)[
            variableName
          ] = variableLocation;
        else if (Object.keys(modelVariables.inputs).includes(variableName))
          (modelVariables.inputs as Record<string, string | undefined>)[
            variableName
          ] = variableLocation;
        else if (Object.keys(modelVariables.outputs).includes(variableName))
          (modelVariables.outputs as Record<string, string | undefined>)[
            variableName
          ] = variableLocation;
        else
          throw new Error(
            `Model ${modelId} does not include variable with name ${variableName}`,
          );
      }
      ...
      for (const variableName in modelVariables.parameters) {
        if (
          ...
          unsetVariables.push(variableName);
      }
      ...
      if (unsetVariables.length > 0)
        throw new Error(
          `Error in model ${modelId}: locations of variables ${JSON.stringify(
            unsetVariables,
          )} was not defined`,
        );
      ...
      // *-------------------------------------------- Entry State --------------------------------------------*/
      const modelEntryState = scxmlDoc.createElementNS(
        "http://www.w3.org/2005/07/scxml",
        "state",
      );
      ...
      // Create on entry
      const startOnEntry = scxmlDoc.createElementNS(
        "http://www.w3.org/2005/07/scxml",
        "onentry",
      );
      ...
      // Create start send
      const startSend = scxmlDoc.createElementNS(
        "http://www.w3.org/2005/07/scxml",
        "send",
      );
      startSend.setAttribute("type", "WoTSimProcessor");
      startSend.setAttribute("target", modelServer);
      const startParamSimID = scxmlDoc.createElementNS(
        "http://www.w3.org/2005/07/scxml",
        "param",
      );
      ...
      const startParamMsgType = scxmlDoc.createElementNS(
        "http://www.w3.org/2005/07/scxml",
        "param",
      );
      ...
      const startParamStepSize = scxmlDoc.createElementNS(
        "http://www.w3.org/2005/07/scxml",
        "param",
      );
      ...
      if (Object.keys(modelVariables.parameters).length > 0) {
        const startParamParameters = scxmlDoc.createElementNS(
          "http://www.w3.org/2005/07/scxml",
          "param",
        );
        ...
        for (const parameter in modelVariables.parameters) {
          dataString += `"${parameter}": ${
            (modelVariables.parameters as Record<string, string>)[parameter]
          },`;
        }
      ...
      if (Object.keys(modelVariables.inputs).length > 0) {
        const startParamInputs = scxmlDoc.createElementNS(
          "http://www.w3.org/2005/07/scxml",
          "param",
        );
        ...
        for (const parameter in modelVariables.inputs) {
          dataString += `"${parameter}": ${
            (modelVariables.inputs as Record<string, string>)[parameter]
          },`;
        }
        dataString = dataString.slice(0, -1);
        dataString += "}";
      ...
      // Create transtion
      const startToPreTick = scxmlDoc.createElementNS(
        "http://www.w3.org/2005/07/scxml",
        "transition",
      );
      ...
      // *------------------------------------------ Pre Tick State ------------------------------------------*/
      const modelPreTickState = scxmlDoc.createElementNS(
        "http://www.w3.org/2005/07/scxml",
        "state",
      );
      ...
      const preTickOnEntry = scxmlDoc.createElementNS(
        "http://www.w3.org/2005/07/scxml",
        "onentry",
      );
      ...
      for (const output in modelVariables.outputs) {
        const startToPreTickAssign = scxmlDoc.createElementNS(
          "http://www.w3.org/2005/07/scxml",
          "assign",
        );
        startToPreTickAssign.setAttribute(
          "location",
          (modelVariables.outputs as Record<string, string>)[output],
        );
        startToPreTickAssign.setAttribute(
          "expr",
          `event.data.outputs["${output}"]`,
        );
      ...
      startToPreTickAssign.setAttribute(
        "location",
        `_${modelId}_responseReceived`,
      );
      startToPreTickAssign.setAttribute("expr", "true");
      preTickOnEntry.appendChild(startToPreTickAssign);

      // Create transition
      const tickTransition = scxmlDoc.createElementNS(
        "http://www.w3.org/2005/07/scxml",
        "transition",
      );
      tickTransition.setAttribute("event", "simulationStep");
      tickTransition.setAttribute(
        "target",
        `_${stateNodeId}_modelTickResponse`,
      );
      modelPreTickState.appendChild(tickTransition);

      // Create transition send
      const tickTransitionSend = scxmlDoc.createElementNS(
        "http://www.w3.org/2005/07/scxml",
        "send",
      );
      ...
      const tickParamSimID = scxmlDoc.createElementNS(
        "http://www.w3.org/2005/07/scxml",
        "param",
      );
      ...
      const tickParamMsgType = scxmlDoc.createElementNS(
        "http://www.w3.org/2005/07/scxml",
        "param",
      );
      ...
      if (Object.keys(modelVariables.inputs).length > 0) {
        const tickParamInputs = scxmlDoc.createElementNS(
          "http://www.w3.org/2005/07/scxml",
          "param",
        );
      ...
      // *------------------------------------------ Tick Response ------------------------------------------*/
      const modelTickResponseState = scxmlDoc.createElementNS(
        "http://www.w3.org/2005/07/scxml",
        "state",
      );
      modelTickResponseState.setAttribute(
        "id",
        `_${stateNodeId}_modelTickResponse`,
      );
      ...
      // Create transition
      const responseTransition = scxmlDoc.createElementNS(
        "http://www.w3.org/2005/07/scxml",
        "transition",
      );
      ...
      // On Entry
      const tickResponseEntry = scxmlDoc.createElementNS(
        "http://www.w3.org/2005/07/scxml",
        "onentry",
      );
      ...
      // Create Response Received true Assign
      const tickResponseAssign = scxmlDoc.createElementNS(
        "http://www.w3.org/2005/07/scxml",
        "assign",
      );
      tickResponseAssign.setAttribute(
        "location",
        `_${modelId}_responseReceived`,
      );
      ...
      const responseReceivedDataElement = scxmlDoc.createElementNS(
        "http://www.w3.org/2005/07/scxml",
        "data",
      );
      responseReceivedDataElement.setAttribute(
        "id",
        `_${modelId}_responseReceived`,
      );
    ...
    if (modelElements.length > 0) {
      ...
      const clockState = scxmlDoc.createElementNS(
        "http://www.w3.org/2005/07/scxml",
        "state",
      );
      ...
      // *------------------------------------------ Pre Tick ------------------------------------------*/
      const clockPreTickState = scxmlDoc.createElementNS(
        "http://www.w3.org/2005/07/scxml",
        "state",
      );
      ...
      // On Entry
      const clockPreTickEntry = scxmlDoc.createElementNS(
        "http://www.w3.org/2005/07/scxml",
        "onentry",
      );
      ...
      // On Entry Send
      const clockPreTickEntrySend = scxmlDoc.createElementNS(
        "http://www.w3.org/2005/07/scxml",
        "send",
      );
      ...
      // Transition
      const clockPreTickTransition = scxmlDoc.createElementNS(
        "http://www.w3.org/2005/07/scxml",
        "transition",
      );
      ...
      // *------------------------------------------ Post Tick ------------------------------------------*/
      const clockPostTickState = scxmlDoc.createElementNS(
        "http://www.w3.org/2005/07/scxml",
        "state",
      );
      ...
      // Transition
      const clockPostTickTransition = scxmlDoc.createElementNS(
        "http://www.w3.org/2005/07/scxml",
        "transition",
      );
      ...
      if (this.XStateMachineOptions.guards === undefined)
        this.XStateMachineOptions.guards = {} satisfies Record<string, string>;
